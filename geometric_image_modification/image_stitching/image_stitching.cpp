...
Stitching::Stitching(int h, int w, int b) {
    height = h;
    width = w;
    BytesPerPixel = b;
    new_height = 1200;
...
void Stitching::readImage(string fileName, unsigned char *image) {
    FILE *file;
...
void Stitching::writeImage(string filename, unsigned char ***final) {
    FILE *file;
...
void Stitching::generateImageL(unsigned char *main_image) {
    long int count{};
...
void Stitching::generateImageM(unsigned char *main_image) {
    long int count{};
...
void Stitching::generateImageR(unsigned char *main_image) {
    long int count{};
...
unsigned char ***Stitching::generateMatrix3D(int h, int w, int b) {
    long int count{};
...
unsigned char ***Stitching::homographicTransformation() {
...
void Stitching::delete3D(unsigned char ***image, int h, int w) {
	for (int i = 0; i < h; i++)
	{
		for (int j = 0; j < w; j++)
			delete[] image[i][j];
